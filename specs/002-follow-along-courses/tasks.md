...
- [x] **1.3** 设计规范落为代码主题（复用现有 theme，确认颜色/字阶足够）
...
- [x] **2.1** `domain/course.ts`：课程进度与完成度
...
- [x] **2.3** `domain/format.ts` 扩展：课程时长与次数格式化
...
- [x] **4.2** `services/courseEngine.ts`：课程执行引擎
...
- [x] **6.4** `(tabs)/me` 与 `(tabs)/index` 统计接入跟练记录
